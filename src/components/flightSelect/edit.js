...
class Edit extends Component {
  ...
  render(){
    const { form:{ getFieldDecorator}, editData:{person,detail, category}, place = [], isChangeTicket = false } = this.props;
    return(
      <div>
        <Row>
          ...
          <Row>
            ...
            <Col span="6">
              {getFieldDecorator('placeFrom_edit',{initialValue: detail.placeFromText})(
                <Select size="small" className="select" placeholder="出發機場"  disabled={isChangeTicket}>
                  {
                    place.map(v => (
            ...
            <Col span="7">
              {getFieldDecorator('placeTo_edit',{initialValue: detail.placeToText})(
                <Select size="small" className="select" placeholder="到達機場"  disabled={isChangeTicket}>
                  {
                    place.map(v => (
                      <Option key={v.Code}>{v.Value}</Option>
                    ))
